...
local blury = L.const(L.array2d(L.fixed(true, 9, 0), 1, 3), {
                      { 1 },
                      { 2 },
                      { 1 }})

local dx = L.const(L.array2d(L.fixed(true, 9, 0), 3, 1), {
                      { 1, 0, -1 }})

local function conv(s, taps)
   local pad_size = im_size
   -- local pad_size = { im_size[1]+16, im_size[2]+3 }
   local I = L.input(L.array2d(L.fixed(true, 9, 0), im_size[1], im_size[2]))
...
local x0 = L.map(L.to_signed())(I)
local x1 = conv({ -1, 0, 3, 1 }, dx)(x0)
